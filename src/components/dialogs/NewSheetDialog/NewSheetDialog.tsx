...
export function NewSheetDialog({ isOpen, onClose }: NewSheetDialogProps) {
  ...
  const handleCreate = () => {
    ...
    if (activeTab === 'templates' && selectedTemplateId) {
      newSheetId = addSheetFromTemplate(selectedTemplateId, name, drawingAssignments);
      ...
      if (titleBlock) {
        newSheetId = addSheet(name, selectedPaperSize as 'A4' | 'A3' | 'A2' | 'A1' | 'A0' | 'Letter' | 'Legal' | 'Tabloid' | 'Custom', 'landscape', selectedTitleBlockId);
      }
    } else {
      return;
    }
    ...
    // Switch to the new sheet and open title block editor for user to fill in details
    if (newSheetId) {
      switchToSheet(newSheetId);
      // Small delay to ensure sheet is loaded before opening editor
      setTimeout(() => {
        setTitleBlockEditorOpen(true);
      }, 100);
    }
  };
